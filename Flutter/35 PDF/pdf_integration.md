...
- **Generation**: delegates to **pure templates** ([data_driven_documents.md](data_driven_documents.md)); **offloads** large docs to an isolate (`compute`), passing serializable model data.
- **Presentation**: uses **`printing`** — `PdfPreview`/`layoutPdf`/`sharePdf` — honoring the passed `PdfPageFormat` ([viewing_and_printing.md](viewing_and_printing.md)).
...
PDF integration is the orchestration seam for documents: pure templates (generation), `printing` (presentation), and the file repository (persistence), unified behind an intent-based service that owns isolate/format/cache concerns. This mirrors the app's other boundaries (data/network/files), keeps document features simple and CI-testable, and scales to new document types by reusing the pipeline — completing the generation→presentation→persistence story ([data_driven_documents.md](data_driven_documents.md), [viewing_and_printing.md](viewing_and_printing.md), [Module 34](../34%20File%20Handling/README.md), [Module 40](../40%20Clean%20Architecture/README.md)).
