...
- **Body**: request payload (JSON) and response body (JSON to parse — [02 · json](../02%20Advanced%20Dart/json_and_serialization.md)).
- **Clients**:
...
Response bodies load into memory (stream large ones); decoded JSON is a heap tree — parse big payloads off-isolate ([02 · isolates](../02%20Advanced%20Dart/isolates.md)).
...
Networking uses platform sockets via the embedder; on the UI isolate for orchestration, heavy parsing should be offloaded ([10 · threading_model](../10%20Flutter%20Architecture/threading_model.md)).

## Dart VM Behavior

Not applicable beyond async on the event loop ([02 · event_loop](../02%20Advanced%20Dart/event_loop.md)).
...
Latency dominates; parallelize independent calls (`Future.wait` — [02 · async_futures](../02%20Advanced%20Dart/async_futures.md)), set timeouts, and offload heavy JSON parsing ([Module 21](../21%20Performance/README.md)).
...
7. **🔴 Where should raw responses be converted to domain models?** — At the repository boundary (DTO→entity), so the UI never sees `Response`/`DioException` ([05 · repository](../05%20Design%20Patterns/repository.md)).
