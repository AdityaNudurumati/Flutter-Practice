...
- **Model as a `Stream`**: expose incoming messages as a broadcast `Stream` the UI/blocs consume ([02 · streams](../02%20Advanced%20Dart/streams.md)); consumers `listen` and **cancel** subscriptions on dispose.
...
- **Lifecycle**: close the channel and cancel subscriptions on dispose; pause on app background ([08 · app_lifecycle](../08%20Widget%20Lifecycle/app_lifecycle.md)).

## Memory Representation

Open connections + stream subscriptions retain callbacks (and captured state) → leaks if not cancelled/closed ([08 · dispose_and_leaks](../08%20Widget%20Lifecycle/dispose_and_leaks.md), [02 · streams](../02%20Advanced%20Dart/streams.md)).
...
Uses platform sockets via the embedder; runs on the isolate's event loop — keep message handlers light ([10 · threading_model](../10%20Flutter%20Architecture/threading_model.md)).
...
| Not pausing on background | Wasted battery/data | Pause/close on app background ([08](../08%20Widget%20Lifecycle/app_lifecycle.md)) |
...
7. **🔴 How should real-time connections behave on app background?** — Pause/close to save battery/data; resume/reconnect on foreground ([08 · app_lifecycle](../08%20Widget%20Lifecycle/app_lifecycle.md)).
