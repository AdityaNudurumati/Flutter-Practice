...
Native WorkManager spins up a background `FlutterEngine` to run the dispatcher, then tears it down ([01_background_execution_model.md](01_background_execution_model.md)).
...
| Expecting iOS to behave like Android | iOS `BGTaskScheduler` runs rarely | iOS-specific expectations ([ios file](04_ios_background_execution.md)) |
...
- Expect **iOS to run rarely** (`BGTaskScheduler` semantics — [04_ios_background_execution.md](04_ios_background_execution.md)); degrade to foreground sync; wrap behind a service.
