...
- **Two integration paths**:
  1. **Via the gateway SDK** (recommended): Stripe/Razorpay payment sheets **already offer** Apple/Google Pay when configured — the wallet token flows through the same `PaymentIntent`/order you already built ([payment_gateways.md](payment_gateways.md)). Least extra work.
...
- **Setup**:
  ...
  - **Apple Pay**: a **Merchant ID + Apple Pay capability/entitlement** in Xcode ([28 · ios_integration](../28%20Native%20iOS/ios_integration.md)), merchant certificate; only on real devices with a card in Wallet.
...
- **Digital goods**: wallets do **not** bypass IAP — you still can't sell coins/subscriptions via Apple/Google Pay; those go through IAP ([in_app_purchases.md](in_app_purchases.md)).
...
Not applicable; Apple Pay needs the capability/entitlement at build ([28 · ios_integration](../28%20Native%20iOS/ios_integration.md)).
...
| Wallets for digital goods | Policy violation | IAP for digital ([in_app_purchases.md](in_app_purchases.md)) |
...
- Configure **Apple Pay merchant id + entitlement** ([28 · ios_integration](../28%20Native%20iOS/ios_integration.md)) / **Google Pay config**; use wallets for **physical/services only** (not IAP goods).
