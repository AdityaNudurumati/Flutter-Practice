...
- **Rail selection (critical, App Store/Play policy)**:
  - **Digital content/services consumed in the app** (coins, pro features, subscriptions, unlockables) → **must** use **in-app purchase** ([in_app_purchases.md](in_app_purchases.md)). Using a gateway for these = rejection/ban.
  - **Physical goods, real-world services** (food delivery, ride, e-commerce, ticket to a physical event) → **must** use a **gateway** (Stripe/Razorpay/wallets) — IAP is **not allowed** for these ([payment_gateways.md](payment_gateways.md)).
