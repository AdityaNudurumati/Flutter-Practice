...
Records carry version/clock metadata; CRDTs carry extra state (e.g., per-replica counters). Manual/keep-both may store multiple versions temporarily ([sync_engine.md](sync_engine.md)).
...
Conflict resolution is where offline-first correctness is won or lost. Choosing per-data strategies (with version-based detection, server authority for critical data, and CRDTs for collaboration) and documenting them prevents silent data loss and surprises. It's a core part of the sync design ([sync_engine.md](sync_engine.md)) and the hardest to retrofit — decide early ([Module 48](../48%20System%20Design/README.md)).
