...
- **Apply immediately**: mutate the local store/state at once; the UI (reading local) reflects it instantly — works offline (queued via outbox — [connectivity_and_outbox.md](connectivity_and_outbox.md)).
...
- **Reconciliation**: when sync pulls server state, it may **replace** the optimistic value with the authoritative one (which usually matches). Conflicts route to conflict resolution ([conflict_resolution.md](conflict_resolution.md)).
- **Idempotency**: pair with client ids so retries don't double-apply ([connectivity_and_outbox.md](connectivity_and_outbox.md)).
...
Optimistic UI is the UX dividend of offline-first: it makes local-source-of-truth feel instant while sync/outbox/conflict handling ensure eventual correctness. Designed with pending/rollback/idempotency and scoped to reversible actions, it delivers native-feeling responsiveness — completing the offline-first stack ([offline_first_fundamentals.md](offline_first_fundamentals.md), [connectivity_and_outbox.md](connectivity_and_outbox.md), [conflict_resolution.md](conflict_resolution.md)).
