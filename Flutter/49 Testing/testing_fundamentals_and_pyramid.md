...
- **Flutter test types (map to pyramid)**:
  ...
  - **Golden test** (widget-level): pixel-snapshot comparison of rendered UI ([widget_and_golden_tests.md](widget_and_golden_tests.md)).
...
- **What NOT to over-test**: trivial getters, framework code, generated code; and don't chase 100% coverage for its own sake ([testing_integration.md](testing_integration.md)).
...
- Keep unit tests **isolated + deterministic** (fakes/mocks at boundaries — [unit_and_mocking.md](unit_and_mocking.md)); leverage **testable architecture** (Clean/MVVM) to push logic into fast unit tests.
