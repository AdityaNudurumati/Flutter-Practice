...
This file covers widget testing (pump/finders/interactions/`pumpWidget` harness, faking dependencies, `pump` vs `pumpAndSettle`) and golden (snapshot) testing (when/how, managing goldens). It's the UI-behavior middle of the pyramid ([01_testing_fundamentals_and_pyramid.md](01_testing_fundamentals_and_pyramid.md)).
...
- **Faking dependencies**: inject **fakes** (fake view model/use case/repository) via the harness (Provider override, constructor, DI) so the widget test is **isolated + deterministic** — no real network ([02_unit_and_mocking.md](02_unit_and_mocking.md)). Test **each UI state** (loading/data/empty/error) by feeding the corresponding state/fake ([Module 43](../43%20MVVM/README.md)).
...
Widget and golden tests are the pyramid's UI-behavior middle: they verify that state renders correctly and interactions work (widget) and that the pixels stay right (golden), device-free and fast enough to run broadly. Driven off faked state (the MVVM payoff), they cover the states/interactions unit tests can't and the visuals E2E is too slow to guard — completing behavioral + visual regression safety before the thin E2E tip ([Module 43](../43%20MVVM/README.md), [04_integration_and_e2e.md](04_integration_and_e2e.md), [01_testing_fundamentals_and_pyramid.md](01_testing_fundamentals_and_pyramid.md)).
