...
- **`MainActivity`** (`kotlin/.../MainActivity.kt`): usually `class MainActivity : FlutterActivity()` — hosts the Flutter engine; where you register channels/plugins ([kotlin_plugin_code.md](kotlin_plugin_code.md)).
- **`AndroidManifest.xml`**: app declaration — package, permissions, activities, intent filters, `application` config ([permissions_and_manifest.md](permissions_and_manifest.md)).
- **`res/`**: launcher icons, `styles.xml` (theme/splash), `strings.xml`, drawables.
- **Flavors**: define `productFlavors` (e.g., `dev`/`prod`) with distinct `applicationId`/config; build with `flutter build apk --flavor prod` + a matching entrypoint (`main_prod.dart`) — pairs with Firebase dev/prod ([18 · firebase_setup_and_core](../18%20Firebase/firebase_setup_and_core.md)).
- **SDK versions**: `minSdk` (lowest supported), `targetSdk` (tested-against), `compileSdk` (build API) — plugins often dictate a floor.

## Memory Representation

Not applicable; build-time config. Release builds enable R8 (shrink/obfuscate) — pairs with `--obfuscate`/`--split-debug-info` ([21 · startup_and_app_size](../21%20Performance/startup_and_app_size.md)).
...
`FlutterActivity` is the Android **embedder** hosting the engine + registering plugins ([10 · embedder_and_startup](../10%20Flutter%20Architecture/embedder_and_startup.md)).
...
| Editing manifest for permissions ad hoc | Missing/duplicated | Declare properly ([permissions_and_manifest.md](permissions_and_manifest.md)) |
...
- Register channels/plugins in **`MainActivity.configureFlutterEngine`** ([kotlin_plugin_code.md](kotlin_plugin_code.md)); keep manifest/permissions correct.

## Performance

Release R8 shrinking + AAB per-device delivery reduce size; correct SDK/flavor config avoids bloat. Build-time only ([21 · startup_and_app_size](../21%20Performance/startup_and_app_size.md)).
