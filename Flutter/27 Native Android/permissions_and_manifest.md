...
- **Manifest** (`android/app/src/main/AndroidManifest.xml`): declare `<uses-permission android:name="android.permission.CAMERA"/>`, `ACCESS_FINE_LOCATION`, etc.; optionally `<uses-feature>` (e.g., camera) — and `<application>`/`<activity>` config, intent filters (deep links — [13 · deep_linking](../13%20Routing/deep_linking_and_url_strategy.md)).
...
Permission dialogs/results go through the Activity (needs `ActivityAware` in plugins — [kotlin_plugin_code.md](kotlin_plugin_code.md)); `permission_handler` manages this.
