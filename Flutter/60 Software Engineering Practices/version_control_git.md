...
- Protect `main` with branch protection: require PR review, green CI (see [CI pipeline and automation](../50%20CI%20CD/ci_pipeline_and_automation.md)), and up-to-date branches.
...
- Tag releases with **semantic versioning** and let CI cut releases from tags (see [delivery and release](./delivery_and_release.md)).
- Review PRs against the team's checklist (see [code quality and review](./code_quality_and_review.md)).
...
5. Cut a release: bump to `v0.2.0` following SemVer, tag it, and write CHANGELOG entries. Wire a placeholder CI check (see [CI pipeline and automation](../50%20CI%20CD/ci_pipeline_and_automation.md)) that must pass before merge.

**Acceptance criteria:** `git log --graph --oneline --all` shows a coherent, mostly-linear history; no build artifacts or secrets are tracked; both PRs are small and single-purpose; the regression commit is identified by bisect; `v0.1.0` and `v0.2.0` tags exist and follow SemVer; the review process matches [code quality and review](./code_quality_and_review.md) and release steps match [delivery and release](./delivery_and_release.md).
