...
1. **Structured logs** — key/value events at flow boundaries with `LogLevel` and a per-flow `correlationId`, PII-scrubbed. (Reuse the `StructuredLogger` above; see [Remote Logging & Observability](../39%20Logging/remote_logging_and_observability.md) for shipping them.)
...
3. **Crash capture** — the three-handler global error net forwarding to a reporter (Crashlytics/Sentry stand-in; see [Crash Reporting](../52%20Monitoring/crash_reporting.md)).
...
**See also:** [Monitoring](../52%20Monitoring/README.md) · [Logging](../39%20Logging/README.md) · [Remote Logging & Observability](../39%20Logging/remote_logging_and_observability.md) · [Crash Reporting](../52%20Monitoring/crash_reporting.md) · [Delivery and Release](./delivery_and_release.md)
