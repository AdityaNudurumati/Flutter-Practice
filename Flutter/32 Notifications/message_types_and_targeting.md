...
- **Targeting** (server-side via Admin SDK / HTTP v1 API):
  - **Token**: send to one device's registration token (personal) — requires your backend to store tokens ([fcm_push.md](fcm_push.md)).
...
Data-message background processing runs in the background isolate ([fcm_push.md](fcm_push.md)); notification-only messages may bypass your Dart code entirely until tapped.
...
Message type + targeting is the contract between your backend and the client push handler. Standardizing on data-only (or hybrid) with a `route`/`type`/`id` schema, server-only sending, and clear token-vs-topic rules makes push predictable and routable, and keeps the display/routing logic in your app (behind the notification service) rather than at the mercy of OS auto-display ([fcm_push.md](fcm_push.md), [handling_and_deeplinks.md](handling_and_deeplinks.md)).
