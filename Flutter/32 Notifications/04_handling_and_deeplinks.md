...
- **Route in payload**: standardize a field — FCM `data['route']` (or `type`+`id` you map to a route), local-notification `payload` string ([01_local_notifications.md](01_local_notifications.md)). Keep it a real app route (`/order/42`) so it maps 1:1 to `go_router`.
...
Tap handling is the bridge from delivery to product value, and its complexity is entirely about state/timing. A `DeepLinkRouter` funnel with cold-start deferral, fed by all reception sources and integrated with `go_router`'s guards, makes notification routing reliable and testable — and reuses the same deep-link infrastructure as universal/app links ([Module 13](../13%20Routing/README.md), [28 · ios_integration](../28%20Native%20iOS/05_ios_integration.md), [05_notifications_integration.md](05_notifications_integration.md)).
