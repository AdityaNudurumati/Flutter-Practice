...
- **`NotificationService` responsibilities**:
  - **Permissions**: request + expose status; degrade gracefully ([27](../27%20Native%20Android/04_permissions_and_manifest.md)/[28](../28%20Native%20iOS/04_infoplist_and_permissions.md)).
  - **FCM**: `getToken`/`onTokenRefresh` → backend; register the top-level background handler before `runApp`; wire `onMessage`/`onMessageOpenedApp`/`getInitialMessage` ([02_fcm_push.md](02_fcm_push.md)).
  - **Foreground rendering**: on `onMessage`, build a **local notification** (channel + payload = route) so foreground push is visible + tappable ([01_local_notifications.md](01_local_notifications.md)).
  - **Deep links**: feed every tap into the `DeepLinkRouter` funnel with cold-start deferral → `go_router` ([04_handling_and_deeplinks.md](04_handling_and_deeplinks.md)).
  - **Topics/scheduling**: `subscribe/unsubscribe`, `scheduleReminder(...)`.
- **Backend owns sending/targeting** (data-only/hybrid, token/topic, server key) ([03_message_types_and_targeting.md](03_message_types_and_targeting.md)); the client never sends.
...
Two isolates as in [02_fcm_push.md](02_fcm_push.md); the service's state lives only in the UI isolate.
