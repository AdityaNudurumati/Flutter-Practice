...
- **Controllers/FocusNodes must be disposed** in `State.dispose` ([Module 08](../08%20Widget%20Lifecycle/README.md), [02 · memory_and_gc](../02%20Advanced%20Dart/memory_and_gc.md)).

## Memory Representation

Controllers/FocusNodes are objects held by `State`; not disposing them leaks ([02 · memory_and_gc](../02%20Advanced%20Dart/memory_and_gc.md)).
...
Text input coordinates with the platform keyboard/IME via the engine/embedder ([06 · architecture_overview](../06%20Flutter%20Fundamentals/architecture_overview.md)).
...
| Using `context` after async submit without `mounted` | Crash | Guard with `if (!mounted) return;` ([06 · build_context](../06%20Flutter%20Fundamentals/build_context.md)) |
...
3. **🟡 Why dispose `TextEditingController`/`FocusNode`?** — They're long-lived objects held by `State`; not disposing leaks memory ([02](../02%20Advanced%20Dart/memory_and_gc.md)).
