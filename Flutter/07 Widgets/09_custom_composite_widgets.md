...
Copy-pasting the same `Container`+`Row`+`Text` block across screens causes drift and bloats `build` methods. Extracting a named composite widget (`UserCard`, `PrimaryButton`) gives reuse, a clear API, `const` rebuild-skipping, and testability — the practical payoff of composition ([03 · composition](../03%20Object%20Oriented%20Programming/06_composition_and_relationships.md)).
...
- Prefer **`StatelessWidget`** unless it owns local state; keep business logic out ([06 · stateless_vs_stateful](../06%20Flutter%20Fundamentals/05_stateless_vs_stateful.md)).
- Custom **`RenderObject`** widgets are for bespoke layout/painting only — rare; see [Module 23](../23%20Custom%20Painting/README.md).

## Memory Representation

Composite widgets are ordinary widgets → elements → render objects; `const` instances are shared/skip rebuilds ([06](../06%20Flutter%20Fundamentals/04_widgets_elements_render_objects.md)).
...
- Compose small widgets; each has a single responsibility ([04 · SRP](../04%20SOLID%20Principles/01_srp_single_responsibility.md)).
