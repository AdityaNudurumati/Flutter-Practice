...
This file covers functional error handling: representing success-or-failure as a typed value (`Result`/`Either`) via sealed classes, why it beats throwing for expected failures, and how to compose it (map/flatMap/fold) across layers. It's the type-system complement to the errors/exceptions distinction ([errors_vs_exceptions.md](errors_vs_exceptions.md)).
...
- **Typed failures**: model failures as a **sealed `Failure` hierarchy** so callers branch and map to UX ([recovery_and_user_errors.md](recovery_and_user_errors.md)) — richer than a string.
