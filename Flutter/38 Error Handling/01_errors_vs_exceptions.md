...
- **Custom exceptions**: implement `Exception`, add fields + a `toString()`; model domain failures (`NetworkException`, `NotFoundException`, `ValidationException`) so callers can branch and map to UX ([04_recovery_and_user_errors.md](04_recovery_and_user_errors.md)).
...
- **Async**: `Future` errors propagate to `await` (catch with try/catch) or `.catchError`; **stream** errors go to `onError`. Uncaught async errors need zone/global handling ([03_global_error_handling.md](03_global_error_handling.md)).
...
Dart has **no checked exceptions** — the compiler doesn't force you to declare/handle throwables; discipline (or `Result` types — [02_result_and_either.md](02_result_and_either.md)) is on you. `assert`s are **debug-only** (stripped in release).
...
Uncaught errors during build/layout/paint go to `FlutterError.onError`; uncaught async/platform errors go to `PlatformDispatcher.onError`/zone ([03_global_error_handling.md](03_global_error_handling.md)).
...
Throwing/catching is cheap relative to I/O but **not free** — don't use exceptions for **normal control flow** in hot paths (prefer `Result` / nullable returns — [02_result_and_either.md](02_result_and_either.md)). `finally` is negligible.
