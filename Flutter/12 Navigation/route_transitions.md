...
Transitions/Hero flights are transient animation objects; the Hero temporarily moves the widget into an overlay during the flight ([09 · compositing](../09%20Rendering%20Pipeline/compositing_and_repaint_boundaries.md)).
...
Transitions are per-frame repaints; wrap expensive content in `RepaintBoundary` if needed ([09 · compositing](../09%20Rendering%20Pipeline/compositing_and_repaint_boundaries.md)). Hero flights composite over an overlay layer.
...
- Wrap heavy transitioning content in `RepaintBoundary` ([09](../09%20Rendering%20Pipeline/compositing_and_repaint_boundaries.md)).
...
- For image Heroes, ensure both ends use similarly-sized/decoded images to avoid flicker and raster spikes ([07 · images](../07%20Widgets/images_and_assets.md)).
