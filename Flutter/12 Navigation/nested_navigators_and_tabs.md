...
- **State preservation** also needs `AutomaticKeepAliveClientMixin` or `IndexedStack` for scroll/list state ([06 · widgets_elements_render_objects](../06%20Flutter%20Fundamentals/widgets_elements_render_objects.md)).

## Memory Representation

`IndexedStack` keeps all tab subtrees (and their nested stacks) alive → higher memory but preserved state; rebuilding tabs saves memory but loses state ([08 · dispose_and_leaks](../08%20Widget%20Lifecycle/dispose_and_leaks.md)).
