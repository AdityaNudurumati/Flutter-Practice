...
- **`context.mounted`** must be checked before using `context` after `await push` ([08 · setstate_mechanics](../08%20Widget%20Lifecycle/setstate_mechanics.md)).

## Memory Representation

The pushed route retains its subtree until popped; results are plain returned values ([navigator_stack.md](navigator_stack.md)).
...
- Pass arguments via the **destination's constructor** (type-safe); reserve untyped route arguments for named routes ([named_routes.md](named_routes.md)).
...
Negligible; each pushed route retains its subtree until popped ([navigator_stack.md](navigator_stack.md)).
