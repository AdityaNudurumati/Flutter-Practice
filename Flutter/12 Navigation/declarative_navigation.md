...
- **Reconciliation**: change the state → rebuild the page list → `Navigator` diffs it (add/remove routes), like widget reconciliation ([06 · widgets_elements_render_objects](../06%20Flutter%20Fundamentals/widgets_elements_render_objects.md)).
...
Pages are immutable descriptions; the `Navigator` maintains the underlying route stack/state ([navigator_stack.md](navigator_stack.md)).
...
Reconciliation is efficient (diff pages like widgets); no notable overhead vs imperative. The cost is conceptual/boilerplate, mitigated by `go_router` ([09 · build_phase](../09%20Rendering%20Pipeline/build_phase.md)).
