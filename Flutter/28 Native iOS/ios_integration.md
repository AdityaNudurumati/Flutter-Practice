...
The iOS embedder ([10 · embedder_and_startup](../10%20Flutter%20Architecture/embedder_and_startup.md)) receives lifecycle/link/notification callbacks in `AppDelegate` and forwards them to Flutter (plugins/channels — [swift_plugin_code.md](swift_plugin_code.md)).
...
- Host a **correct AASA** (HTTPS, `.well-known`, right type/App ID/paths) and add **Associated Domains**; route links via `go_router` ([13 · deep_linking](../13%20Routing/deep_linking_and_url_strategy.md)).
...
- Handle links/notifications in `AppDelegate` and forward to Flutter behind a repository ([swift_plugin_code.md](swift_plugin_code.md)).
