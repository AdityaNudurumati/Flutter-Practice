...
The build phase re-runs `build` for dirty elements ([09 · build_phase](../09%20Rendering%20Pipeline/build_phase.md)). If a change rebuilds a large subtree (or rebuilds happen too often), the UI thread overruns its budget. This file covers the three levers to keep rebuilds small and cheap.
...
Three levers:
1. **`const` widgets**: `const` subtrees are canonicalized/identical across builds, so the element **skips** rebuilding them ([09 · build_phase](../09%20Rendering%20Pipeline/build_phase.md)).
2. **Scope via widget classes + state pushdown**: extract **widget classes** (not `_buildX()` methods — [07 · custom_composite_widgets](../07%20Widgets/custom_composite_widgets.md)) and put `setState`/state in the **smallest** widget that owns the changing UI, so only it rebuilds.
...
Rebuilds allocate throwaway widgets (GC'd); `const` avoids allocation; elements/render objects persist. Fewer/scoped rebuilds = less allocation churn ([02 · memory_and_gc](../02%20Advanced%20Dart/memory_and_gc.md)).
...
Not applicable beyond build feeding the pipeline; heavy `build` work blocks the UI thread ([02 · isolates](../02%20Advanced%20Dart/isolates.md)).
...
Build cost ∝ dirty-subtree size × per-widget cost × rebuild frequency. Scoping + `const` + selectors attack all three — the primary UI-bound perf lever ([profiling_and_frame_budget.md](profiling_and_frame_budget.md)).
