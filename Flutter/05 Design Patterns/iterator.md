...
- **`sync*` generators** are the idiomatic Dart way to produce lazy iterables without hand-writing an iterator ([01 · collections](../01%20Dart%20Fundamentals/collections.md)).
...
6. **🔴 `sync*` vs `async*`?** — `sync*` produces a lazy `Iterable` (pull); `async*` produces a `Stream` (push, async) ([02 · streams](../02%20Advanced%20Dart/streams.md)).
