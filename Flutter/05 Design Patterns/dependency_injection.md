...
Hard-wired dependencies (`final repo = FirebaseRepo()`) make code untestable and rigid ([Module 04 · DIP](../04%20SOLID%20Principles/dip_dependency_inversion.md)). DI decouples "what a class needs" from "who provides it," enabling substitution (real vs fake), single wiring point, and clear dependency graphs.
...
Container-managed singletons live for app lifetime; factory registrations create per-request instances. Mind retained singletons ([02 · memory_and_gc](../02%20Advanced%20Dart/memory_and_gc.md)).
...
- Register single instances via the container instead of hard singletons ([singleton.md](singleton.md)).
