...
- **Dart side**:
  ...
  - `listen(onData, onError, onDone)`; **cancel** the subscription to stop (and to avoid leaks — [08 · dispose_and_leaks](../08%20Widget%20Lifecycle/05_dispose_and_leaks.md)). Errors arrive as `PlatformException` on the stream.
...
The native observer/receiver + the Dart subscription are live resources; not cancelling leaks both (and keeps native sensors running/battery drain) ([08 · dispose_and_leaks](../08%20Widget%20Lifecycle/05_dispose_and_leaks.md)).
...
Events cross the platform/UI runners; native emits on the main thread (marshal sensor callbacks) ([10 · threading_model](../10%20Flutter%20Architecture/03_threading_model.md)).

## Dart VM Behavior

Stream events dispatch on the root isolate's event loop; keep handlers light ([02 · streams](../02%20Advanced%20Dart/03_streams.md)).
...
Push-based is efficient vs polling; native sensor/observer registration costs battery — stop on cancel/background ([08 · app_lifecycle](../08%20Widget%20Lifecycle/06_app_lifecycle.md)). Keep handlers light and events small ([Module 21](../21%20Performance/README.md)).
...
- Pause/stop streams on app background for battery ([08 · app_lifecycle](../08%20Widget%20Lifecycle/06_app_lifecycle.md)); resume on foreground.
