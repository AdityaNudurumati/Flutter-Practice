...
Most native integration is discrete calls with a result. `MethodChannel` standardizes this: a named channel + method dispatch + typed-ish result/error, async over the channel infrastructure ([platform_channel_fundamentals.md](platform_channel_fundamentals.md)).
...
Small serialized messages both ways; large data should be `Uint8List` or avoided ([platform_channel_fundamentals.md](platform_channel_fundamentals.md)).

## Compiler Behavior

`invokeMethod<T>` returns `T?` but isn't truly type-checked end-to-end (native returns are dynamic) — Pigeon adds real type-safety ([plugins_and_pigeon.md](plugins_and_pigeon.md)).
...
Message crosses platform/UI runners; native handler runs on the platform main thread ([10 · threading_model](../10%20Flutter%20Architecture/threading_model.md)).

## Dart VM Behavior

Dart caller awaits on the root isolate; background isolates need messenger setup ([02 · isolates](../02%20Advanced%20Dart/isolates.md)).
...
- Namespace channels; for type-safety across the boundary, use **Pigeon** ([plugins_and_pigeon.md](plugins_and_pigeon.md)).
...
**Battery + share bridge (Flutter + native):** Implement a `MethodChannel` for `getBatteryLevel` and `share`, native handlers (Kotlin/Swift) that offload work and return results/errors, and a `BatteryRepository` exposing a typed Dart API with error mapping (incl. `MissingPluginException`). Acceptance: request/reply works; errors mapped; native work off main thread; repository-wrapped; runs on a device. (Streams continue in [event_channel.md](event_channel.md).)
