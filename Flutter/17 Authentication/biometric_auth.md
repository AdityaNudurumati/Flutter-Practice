...
- **Role**: a **local gate** — on success, reveal the app or read the securely-stored session token ([15 · secure_storage](../15%20Local%20Storage/secure_storage.md)) / authorize a sensitive action. It does **not** authenticate to your server.
...
No biometric data is exposed to your app (OS-handled); you only get pass/fail. The gated secret lives in secure storage ([15](../15%20Local%20Storage/secure_storage.md)).
...
`local_auth` calls native biometric APIs via the embedder ([10 · embedder_and_startup](../10%20Flutter%20Architecture/embedder_and_startup.md)); requires platform config (Face ID usage string on iOS, permissions on Android).
...
- Pair with app-lifecycle handling (re-lock on background) for sensitive apps ([08 · app_lifecycle](../08%20Widget%20Lifecycle/app_lifecycle.md)).

## Architect Perspective

Biometrics are a UX-security convenience layer over the real session, not an identity provider. Architecturally they gate access to secure-stored tokens and sensitive actions, with mandatory fallback and platform config, complementing token auth and secure storage ([token_auth_jwt.md](token_auth_jwt.md), [15 · secure_storage](../15%20Local%20Storage/secure_storage.md), [Module 37](../37%20Security/README.md)).
