...
Impeller exists largely to eliminate one classic GPU-bound stall: Skia compiled shaders *on demand* at runtime (first-frame jank); Impeller precompiles them. See [rasterization with Skia and Impeller](../09%20Rendering%20Pipeline/06_rasterization_skia_impeller.md).
...
For Flutter-specific memory tuning, see [Performance: memory optimization](../21%20Performance/04_memory_optimization.md).
