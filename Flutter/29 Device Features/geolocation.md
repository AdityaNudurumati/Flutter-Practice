...
- **Preflight**: `isLocationServiceEnabled()` (user can toggle GPS off) → `checkPermission()`/`requestPermission()`. iOS distinguishes **whileInUse** vs **always** (background needs always + justification — [28 · ios_integration](../28%20Native%20iOS/ios_integration.md)); Android has coarse vs fine.
