...
All prefs are loaded into an in-memory map on first access; keep the dataset **small**. Native backing is a plist/XML file (readable on a rooted/jailbroken device) — hence no secrets ([secure_storage.md](secure_storage.md)).
...
Crosses the embedder to native prefs storage ([10 · embedder_and_startup](../10%20Flutter%20Architecture/embedder_and_startup.md)).
...
| Mistake | Why | Fix |
|---------|-----|-----|
| Storing tokens/secrets | Plain text, readable | Secure storage ([secure_storage.md](secure_storage.md)) |
