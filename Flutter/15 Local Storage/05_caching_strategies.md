...
- Wrap in a **repository** that hides the strategy from callers ([05 · repository](../05%20Design%20Patterns/20_repository.md)).

## Memory Representation

Cache entries hold data + metadata (timestamp/etag). Memory caches must be **bounded** (LRU/size cap) to avoid leaks; disk caches use the temporary dir ([04_file_storage.md](04_file_storage.md), [02 · memory_and_gc](../02%20Advanced%20Dart/13_memory_and_gc.md)).
...
| Unbounded memory cache | Leak/OOM | LRU/size cap ([02](../02%20Advanced%20Dart/13_memory_and_gc.md)) |
...
Caching cuts latency and network cost and enables offline reads; SWR gives instant-then-fresh UX. Bound caches to protect memory; parse large cached payloads off-isolate ([02 · isolates](../02%20Advanced%20Dart/04_isolates.md), [Module 21](../21%20Performance/README.md)).
