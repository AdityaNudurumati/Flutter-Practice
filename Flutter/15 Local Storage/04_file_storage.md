...
- **Large files/heavy parsing**: read/parse off the UI isolate (`Isolate.run`/streams) to avoid jank ([02 · isolates](../02%20Advanced%20Dart/04_isolates.md)).

## Memory Representation

Files live on disk; `readAsBytes`/`readAsString` load into memory (mind large files — stream them). Cache dirs are OS-purgeable, so never store must-keep data there ([02 · memory_and_gc](../02%20Advanced%20Dart/13_memory_and_gc.md)).
...
`path_provider` crosses the embedder to native path APIs; file I/O uses the IO task runner conceptually ([10 · threading_model](../10%20Flutter%20Architecture/03_threading_model.md)).
...
| Reading huge files on UI isolate | Jank | Stream/`Isolate.run` ([02](../02%20Advanced%20Dart/04_isolates.md)) |
...
- Encrypt sensitive files if needed ([Module 37](../37%20Security/README.md)); use files for cache with TTL ([05_caching_strategies.md](05_caching_strategies.md)).
