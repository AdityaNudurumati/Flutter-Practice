...
Once you have a path ([01_app_directories_and_paths.md](01_app_directories_and_paths.md)), you read/write bytes or text. This file covers the `File` API for text/bytes/JSON, async vs sync, streaming large files, atomic writes, and offloading heavy work — the mechanics of not blocking the UI or losing data.
...
- **Offload heavy CPU** (parsing/transform of big data) to an **isolate** via `compute()`/`Isolate.run` so decoding doesn't jank ([02 · isolates](../02%20Advanced%20Dart/04_isolates.md)).
...
- Create parent dirs with `create(recursive: true)`; check `exists()`; pair with the correct directory ([01_app_directories_and_paths.md](01_app_directories_and_paths.md)).
