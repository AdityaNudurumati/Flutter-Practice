...
- **Cache key**: a **stable id** from the URL (e.g., SHA-1 hash) + extension → filename in the cache dir ([01_app_directories_and_paths.md](01_app_directories_and_paths.md)). Deterministic lookup, no collisions.
- **Streamed download**: `dio.download(url, savePath, onReceiveProgress: (r, t) => ...)` streams to disk with **progress** (never loads the whole file in memory) ([Module 16](../16%20Networking/README.md)); write to a **temp file then rename** into place (atomic — [02_reading_writing_files.md](02_reading_writing_files.md)) so a partial download isn't served.
...
- **OS may clear cache**: entries can vanish → always able to **re-fetch**; never store non-re-fetchable data in cache ([01_app_directories_and_paths.md](01_app_directories_and_paths.md)).
...
- Treat cache as **re-fetchable** (never the only copy — OS clears it); wrap behind a repository ([05_file_integration.md](05_file_integration.md)).
