...
- **Immutability + `Equatable`/`freezed`** on states enables `buildWhen` and correct rebuilds ([02 · immutability](../02%20Advanced%20Dart/10_immutability.md)).

## Memory Representation

The bloc holds current state + a state stream; `BlocProvider` disposes it (closing streams). States are immutable value objects ([08 · dispose_and_leaks](../08%20Widget%20Lifecycle/05_dispose_and_leaks.md)).

## Compiler Behavior

Sealed event/state families give exhaustive `switch` handling ([02 · records_and_patterns](../01%20Dart%20Fundamentals/09_records_and_patterns.md)); missing-bloc lookup is a runtime error (like Provider).
...
Not applicable beyond rebuilds; async runs on the event loop ([02 · event_loop](../02%20Advanced%20Dart/01_event_loop.md)).
...
- Prefer **Cubit** ([08_cubit.md](08_cubit.md)) when events add no value (simpler API).

## Performance

Rebuild control via `buildWhen`/`BlocSelector` and value-equal states; unaffected widgets don't rebuild. State stream overhead is negligible; the discipline cost is boilerplate ([09 · build_phase](../09%20Rendering%20Pipeline/02_build_phase.md)).
...
7. **🔴 BLoC vs Cubit?** — Cubit drops events and exposes methods that `emit` directly — simpler, less boilerplate; BLoC's events add traceability/replayability for complex flows ([08_cubit.md](08_cubit.md)).
