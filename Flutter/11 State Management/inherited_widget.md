...
Lifting state up + prop-drilling doesn't scale ([setstate_and_lifting_state.md](setstate_and_lifting_state.md)). `InheritedWidget` lets any descendant read ancestor data directly and subscribes it for targeted rebuilds — decoupling access from tree distance while keeping rebuilds efficient. It's *the* framework primitive for shared state.
...
- `dependOn...` **registers** the calling element as a dependent and returns the data ([06 · build_context](../06%20Flutter%20Fundamentals/build_context.md)).
...
The `InheritedWidget` is part of the tree; the element tracks its **dependent elements** for targeted rebuilds. Data should be immutable/value-equal for correct `updateShouldNotify` ([03 · equality_and_copying](../03%20Object%20Oriented%20Programming/equality_and_copying.md)).
...
| Reading with a context above the widget | Not found | Use a `Builder`/descendant context ([06](../06%20Flutter%20Fundamentals/build_context.md)) |
...
Targeted rebuilds (only dependents) are the key win vs lifting's subtree rebuilds. `InheritedModel` narrows further to aspects. Correct `updateShouldNotify` prevents needless rebuilds ([09 · build_phase](../09%20Rendering%20Pipeline/build_phase.md)).
