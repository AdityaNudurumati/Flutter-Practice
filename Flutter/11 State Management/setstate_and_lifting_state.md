...
- **`setState`** ([08 · setstate_mechanics](../08%20Widget%20Lifecycle/setstate_mechanics.md)): mutate local state + mark dirty → rebuild this subtree.
...
State lives in the ancestor's `State`; children are stateless (throwaway widgets). No extra machinery ([06 · widgets_elements_render_objects](../06%20Flutter%20Fundamentals/widgets_elements_render_objects.md)).

## Compiler Behavior

`const` children unaffected by the state can be skipped ([09 · build_phase](../09%20Rendering%20Pipeline/build_phase.md)).
...
| Prop-drilling through many layers | Verbose, fragile | Use `InheritedWidget`/a solution ([inherited_widget.md](inherited_widget.md)) |
...
- Extract subwidgets + `const` to limit rebuild scope ([09 · build_phase](../09%20Rendering%20Pipeline/build_phase.md)).
