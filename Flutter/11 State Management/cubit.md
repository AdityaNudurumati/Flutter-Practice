...
Cubit holds current state + a state stream; `BlocProvider` disposes it. Immutable states ([02 · immutability](../02%20Advanced%20Dart/immutability.md)).
...
Same as BLoC: value-equal states + `BlocSelector`/`buildWhen` give tight rebuilds; less boilerplate than BLoC with equal runtime characteristics ([09 · build_phase](../09%20Rendering%20Pipeline/build_phase.md)).
