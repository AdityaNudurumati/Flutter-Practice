...
Framework objects (the trees) live in the Dart heap; the engine manages GPU resources/textures; each runs with its own memory concerns ([02 · memory_and_gc](../02%20Advanced%20Dart/13_memory_and_gc.md)).

## Compiler Behavior

Framework Dart compiles JIT (debug)/AOT (release); the engine is precompiled C++ shipped with the app ([02 · dart_compilation](../02%20Advanced%20Dart/14_dart_compilation.md)).
...
The engine hosts the Dart runtime; your app runs on the root isolate. `compute`/isolates spawn additional isolates for parallel work ([02 · isolates](../02%20Advanced%20Dart/04_isolates.md)).
