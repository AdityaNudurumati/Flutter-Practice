...
- Requires **JIT** → debug only. Release (AOT) can't hot reload ([02 · dart_compilation](../02%20Advanced%20Dart/14_dart_compilation.md)).
...
Hot reload preserves the existing Element/`State` objects (state lives there — [04_widgets_elements_render_objects.md](04_widgets_elements_render_objects.md)); only code is swapped. Hot restart discards them.

## Compiler Behavior

JIT recompiles changed libraries to Kernel and injects; the CFE/VM handle incremental compilation ([02 · dart_compilation](../02%20Advanced%20Dart/14_dart_compilation.md)).
...
- Keep `build` pure so reload re-projects UI cleanly from state ([03_declarative_ui.md](03_declarative_ui.md)).
- Verify cold-start/init logic with hot restart or a fresh launch, not reload.

## Performance

Hot reload is sub-second; hot restart is a few seconds; full rebuild is longest. None reflect *release* performance — benchmark in release ([02 · dart_compilation](../02%20Advanced%20Dart/14_dart_compilation.md)).
...
2. **🟢 Why does hot reload work in debug but not release?** — Debug uses the JIT-capable VM (can inject code); release is AOT native with no JIT ([02](../02%20Advanced%20Dart/14_dart_compilation.md)).
...
4. **🟡 What preserves state across hot reload?** — The persistent Element/`State` objects; only code is swapped ([04_widgets_elements_render_objects.md](04_widgets_elements_render_objects.md)).
