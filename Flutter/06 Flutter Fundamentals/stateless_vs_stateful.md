...
The `State` object lives with the Element (persistent). The widget objects (both kinds) are throwaway config recreated each build ([widgets_elements_render_objects.md](widgets_elements_render_objects.md)).

## Compiler Behavior

`const` constructors on stateless widgets enable skip-rebuild optimizations ([declarative_ui.md](declarative_ui.md)). `State` fields being mutable is expected (not `final`).
