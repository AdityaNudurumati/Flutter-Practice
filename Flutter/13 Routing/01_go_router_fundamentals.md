...
Raw Navigator 2.0 is powerful but verbose ([12 · declarative_navigation](../12%20Navigation/04_declarative_navigation.md)); imperative named routes don't sync URLs or handle deep links well. `go_router` gives a concise, URL-first API that's the officially-recommended production choice.
...
`go_router` manages the Navigator 2.0 delegate/pages; route state is derived from the current location ([12 · declarative_navigation](../12%20Navigation/04_declarative_navigation.md)).

## Compiler Behavior

Path strings are untyped by default (typos = runtime 404) → use **type-safe routes** for compile safety ([05_type_safe_and_error_handling.md](05_type_safe_and_error_handling.md)).
...
| Untyped path typos | Runtime 404 | Use type-safe routes ([05_type_safe_and_error_handling.md](05_type_safe_and_error_handling.md)) |
...
Comparable to Navigator; route matching is cheap. Deep-link restoration builds the matched stack once ([12 · navigator_stack](../12%20Navigation/01_navigator_stack.md)).
...
6. **🔴 Why prefer type-safe routes?** — Untyped path strings cause runtime 404 on typos; codegen'd typed routes catch errors at compile time ([05_type_safe_and_error_handling.md](05_type_safe_and_error_handling.md)).
