...
`indexedStack` keeps all branches alive (state/stacks preserved) — memory ∝ number/heft of branches, like Module 12's `IndexedStack` ([12 · nested_navigators_and_tabs](../12%20Navigation/05_nested_navigators_and_tabs.md), [08 · dispose_and_leaks](../08%20Widget%20Lifecycle/05_dispose_and_leaks.md)).
...
`indexedStack` preserves all branches (memory cost) for instant, state-preserving switches — same tradeoff as manual `IndexedStack`, now with deep-link support ([12 · nested_navigators_and_tabs](../12%20Navigation/05_nested_navigators_and_tabs.md)).
