...
`redirect` runs per navigation and on every `refreshListenable` notification — keep it O(1)/pure (no I/O). Negligible if disciplined ([09 · build_phase](../09%20Rendering%20Pipeline/02_build_phase.md)).
