...
- Set up the engine's **task runners/threads** ([03_threading_model.md](03_threading_model.md)).
- Feed **input events** (touch/keyboard/mouse) into the engine → `GestureBinding`.
- Manage **app lifecycle** signals (foreground/background) → `AppLifecycleState` ([08 · app_lifecycle](../08%20Widget%20Lifecycle/06_app_lifecycle.md)).
...
5. `WidgetsFlutterBinding.ensureInitialized()` sets up bindings ([02_framework_stack.md](02_framework_stack.md)).
...
Embedder holds the platform surface/window handles; engine + Dart heap initialized during startup ([04_engine_internals.md](04_engine_internals.md)).

## Compiler Behavior

Release apps load an **AOT snapshot**; the embedder starts the engine which executes it ([02 · dart_compilation](../02%20Advanced%20Dart/14_dart_compilation.md)).
...
The engine is created/hosted by the embedder; it begins the frame loop once `runApp` schedules a frame ([09 · scheduler](../09%20Rendering%20Pipeline/07_scheduler_and_vsync.md)).

## Dart VM Behavior

The embedder/engine initializes the Dart runtime; `main` runs on the root isolate's event loop ([02 · event_loop](../02%20Advanced%20Dart/01_event_loop.md)).
