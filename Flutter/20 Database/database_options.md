...
- **Just a few key-values / secrets / blobs** → not a DB — use prefs/secure/files ([15](../15%20Local%20Storage/storage_options_overview.md)).

## Memory Representation

DBs store on disk; queries load result sets into memory (bound with `LIMIT`/pagination). Reactive engines re-emit on change ([02 · streams](../02%20Advanced%20Dart/streams.md), [02 · memory_and_gc](../02%20Advanced%20Dart/memory_and_gc.md)).
...
DB plugins cross the embedder to native SQLite/engine; some (Drift/Isar) can run on background isolates for heavy work ([02 · isolates](../02%20Advanced%20Dart/isolates.md)).
...
| Unbounded queries | Memory/perf | `LIMIT`/pagination + indexes ([modeling_migrations_performance.md](modeling_migrations_performance.md)) |
...
Indexed, bounded queries are fast; typed engines avoid parse/cast overhead; reactive streams re-run only affected queries. Heavy work off-isolate keeps the UI smooth ([modeling_migrations_performance.md](modeling_migrations_performance.md), [Module 21](../21%20Performance/README.md)).
