...
- **Open**: `openDatabase(path, version:, onCreate:, onUpgrade:, onConfigure:)` (path via `path_provider`/`getDatabasesPath`). `onCreate` builds the schema; `onUpgrade` migrates on version bumps ([05_modeling_migrations_performance.md](05_modeling_migrations_performance.md)).
...
- **No reactivity**: `sqflite` doesn't stream changes — you re-query manually (or add your own change notifications). This (plus untyped rows) is why big apps prefer Drift ([03_drift.md](03_drift.md)).
- **Repository**: map rows→entities at the boundary; keep SQL out of the UI ([05 · repository](../05%20Design%20Patterns/20_repository.md)).

## Memory Representation

Query results are lists of maps in memory — bound with `LIMIT`/pagination. The DB is a single file on disk ([15 · file_storage](../15%20Local%20Storage/04_file_storage.md)).
...
Crosses the embedder to native SQLite; runs on a background thread internally but marshals via the plugin ([02 · isolates](../02%20Advanced%20Dart/04_isolates.md)).
...
- For typed queries + reactivity, prefer **Drift** (built on SQLite) ([03_drift.md](03_drift.md)).

## Performance

Fast for indexed, bounded queries; transactions batch writes efficiently. Untyped mapping adds a little overhead; offload heavy queries; index hot columns ([05_modeling_migrations_performance.md](05_modeling_migrations_performance.md)).
