...
This file covers configuring `logger` (filter/printer/output), the dev-vs-prod split, and — most importantly — wrapping it behind an abstraction injected via DI so logging is centralized, swappable, and environment-aware. It's the practical setup layer over the fundamentals ([01_logging_fundamentals.md](01_logging_fundamentals.md)).
...
- **Prod config**: filter at **info/warning+**, a **compact/structured (JSON) printer**, output to a **remote sink** or a **no-op** (to strip noise) — lean, searchable, PII-safe ([03_production_logging_and_redaction.md](03_production_logging_and_redaction.md)).
...
Filtering below threshold before formatting keeps prod cheap; PrettyPrinter is dev-only. The facade adds negligible indirection. Remote/buffering outputs should batch (async) so logging never blocks — covered in [04_remote_logging_and_observability.md](04_remote_logging_and_observability.md).
