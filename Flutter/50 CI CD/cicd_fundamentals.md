...
- **Triggers**: pipelines run on events — **PR** (analyze + fast tests), **push to main/merge** (full build), **tag/release** (build + sign + deploy), **schedule** (nightly E2E). Different triggers → different stages ([ci_pipeline_and_automation.md](ci_pipeline_and_automation.md)).
- **Core principles**:
...
Pipeline speed is the KPI: fast PR feedback (analyze + unit/widget in minutes) keeps developers in flow; caching + incremental/monorepo runs keep CI fast at scale ([ci_pipeline_and_automation.md](ci_pipeline_and_automation.md)/[Module 45](../45%20Modular%20Architecture/README.md)). Slow pipelines get bypassed; fast, reliable ones get trusted.
