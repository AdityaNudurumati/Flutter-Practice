...
- **PR trigger — fast CI + gate** ([02_ci_pipeline_and_automation.md](02_ci_pipeline_and_automation.md)): pinned SDK → cached `pub get` → **`dart format` + `flutter analyze`** → **`flutter test --coverage`** (unit/widget), parallel + **`melos --since`** incremental (monorepo). **Merge gated** on green (protected branch + CODEOWNERS — [Module 47](../47%20Scalable%20Applications/README.md)). E2E on nightly/merge (not every PR).
- **Merge trigger — build + beta**: build **signed** artifacts per **flavor** ([03_build_signing_and_flavors.md](03_build_signing_and_flavors.md)) with an **auto-bumped unique build number**; **auto-deploy to beta** (Play internal / TestFlight) with **generated release notes** ([04_cd_release_automation.md](04_cd_release_automation.md)). This is Continuous Delivery to staging.
