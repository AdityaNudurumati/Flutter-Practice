...
Each layer (especially cached `RepaintBoundary` textures) uses GPU memory. Too many boundaries waste memory; too few cause broad repaints ([02 · memory_and_gc](../02%20Advanced%20Dart/13_memory_and_gc.md)).
...
The raster thread rasterizes each layer; a stable `RepaintBoundary` can be rasterized once and reused across frames (saving raster time). Excessive layers increase compositing overhead ([06_rasterization_skia_impeller.md](06_rasterization_skia_impeller.md)).
...
- Distinguish **rebuild** (build phase — fix with `const`/scoping — [02_build_phase.md](02_build_phase.md)) from **repaint** (paint/composite — fix with `RepaintBoundary`).
