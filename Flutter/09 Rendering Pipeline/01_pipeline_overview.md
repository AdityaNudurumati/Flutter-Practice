...
The frame steps (driven by `SchedulerBinding` — [07_scheduler_and_vsync.md](07_scheduler_and_vsync.md)):
1. **Vsync** signals a new frame.
2. **Animate:** tickers/animations advance; callbacks may mark things dirty.
3. **Build:** dirty elements rebuild; widget→element→render tree updates ([02_build_phase.md](02_build_phase.md)).
4. **Layout:** render objects compute sizes from constraints ([03_layout_phase.md](03_layout_phase.md)).
5. **Paint:** render objects record drawing commands into **layers** ([04_paint_phase.md](04_paint_phase.md)).
6. **Composite:** the layer tree is assembled ([05_compositing_and_repaint_boundaries.md](05_compositing_and_repaint_boundaries.md)).
7. **Rasterize:** the engine's raster thread converts layers to pixels via Skia/Impeller ([06_rasterization_skia_impeller.md](06_rasterization_skia_impeller.md)).
...
The three trees (widget/element/render) live in the Dart heap; the layer tree and GPU textures live engine-side ([06](../06%20Flutter%20Fundamentals/04_widgets_elements_render_objects.md), [02 · memory_and_gc](../02%20Advanced%20Dart/13_memory_and_gc.md)).

## Compiler Behavior

`const` widgets reduce build-phase work (skipped subtrees) ([06 · declarative_ui](../06%20Flutter%20Fundamentals/03_declarative_ui.md)).
...
The engine receives the layer tree and rasterizes on the raster thread; long raster times (heavy effects, shader compilation) drop frames even if the UI thread was fast ([06_rasterization_skia_impeller.md](06_rasterization_skia_impeller.md)).

## Dart VM Behavior

Build/layout/paint run as Dart on the root isolate's frame callback; blocking the isolate (heavy sync work) delays the whole pipeline ([02 · isolates](../02%20Advanced%20Dart/04_isolates.md)).
...
| Heavy sync work in build/handlers | Blocks UI thread → misses budget | Offload to isolates ([02](../02%20Advanced%20Dart/04_isolates.md)) |
