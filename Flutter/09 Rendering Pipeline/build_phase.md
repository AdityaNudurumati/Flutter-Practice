...
- Rebuilding an element re-runs its widget's `build`, producing new child widgets; `Element.updateChild` **reconciles** each: same `runtimeType`+`key` → update in place; else → replace subtree ([06](../06%20Flutter%20Fundamentals/widgets_elements_render_objects.md)).
...
New widget objects are allocated each rebuild (throwaway, GC'd); elements/render objects persist ([02 · memory_and_gc](../02%20Advanced%20Dart/memory_and_gc.md)).
...
Frequent small allocations (widgets) are cheap; huge rebuilds create GC pressure ([02 · memory_and_gc](../02%20Advanced%20Dart/memory_and_gc.md)).
...
| Build-helper methods instead of widget classes | No `const`/scoping | Extract widget classes ([07 · custom_composite_widgets](../07%20Widgets/custom_composite_widgets.md)) |
