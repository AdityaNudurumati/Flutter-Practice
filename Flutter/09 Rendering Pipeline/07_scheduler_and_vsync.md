...
- Frame callback phases:
  ...
  - **Persistent callback**: the rendering pipeline (build → layout → paint → composite) ([01_pipeline_overview.md](01_pipeline_overview.md)).
...
- Blocking the UI isolate delays the frame callback → dropped frames ([02 · event_loop](../02%20Advanced%20Dart/01_event_loop.md)).

## Memory Representation

Callbacks are closures held by the binding; one-shot post-frame callbacks are cleared after running. Persistent tickers must be disposed ([08 · dispose_and_leaks](../08%20Widget%20Lifecycle/05_dispose_and_leaks.md)).
...
The engine's vsync waiter triggers the Dart frame callback; UI-thread frame work must finish before the raster thread can present on time ([06_rasterization_skia_impeller.md](06_rasterization_skia_impeller.md)).

## Dart VM Behavior

Frame callbacks run on the root isolate's event loop; long synchronous tasks there block frame production — offload to isolates ([02 · isolates](../02%20Advanced%20Dart/04_isolates.md)).
...
| Not disposing `AnimationController`/tickers | Leaks; keeps frames scheduled | `dispose()` them ([08](../08%20Widget%20Lifecycle/05_dispose_and_leaks.md)) |
