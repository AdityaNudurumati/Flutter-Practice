...
Layout is the second phase: the render tree computes geometry. This file covers the render-object layout algorithm (the internals behind the widget-level rule in [07 · constraints_and_sizing](../07%20Widgets/constraints_and_sizing.md)), `performLayout`, `parentUsesSize`, intrinsic dimensions, and **relayout boundaries** that keep layout incremental.
...
- **Sliver layout** uses `SliverConstraints`/`SliverGeometry` (scroll offset, remaining space) instead of `BoxConstraints` ([07 · scrolling_and_slivers](../07%20Widgets/scrolling_and_slivers.md)).
...
Layout runs on the UI thread as part of the frame; excessive layout (deep trees, intrinsics, unbounded scroll misuse) shows as high `buildDuration` ([pipeline_overview.md](pipeline_overview.md)).
...
| Unbounded constraints to scrollables | Layout error | Bound them ([07 · constraints_and_sizing](../07%20Widgets/constraints_and_sizing.md)) |
