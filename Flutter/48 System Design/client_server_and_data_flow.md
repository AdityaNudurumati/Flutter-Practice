...
This file covers designing the contract + data flow: API style, pagination, real-time vs polling, payload shaping, and the read/write flows. It's the "how app and backend talk" layer of the design process ([system_design_fundamentals.md](system_design_fundamentals.md)), building on networking ([Module 16](../16%20Networking/README.md)).
...
- **Read flow**: request → (cache-first check) → network → **cache** → map DTO→entity → render; show cached data immediately, refresh in background (stale-while-revalidate — [caching_offline_and_scale.md](caching_offline_and_scale.md)).
