...
- **Invariant:** a rule that must always hold (balance ≥ 0). Methods enforce it; construction validates it (asserts/initializer list — see [02 · constructors](../02%20Advanced%20Dart/constructors_and_singletons.md)).

## Memory Representation

Not applicable beyond normal object layout — privacy is a compile-time visibility rule, not a runtime tag ([classes_and_objects.md](classes_and_objects.md)).
...
| Exposing internal mutable collection | Callers mutate internals | Return `List.unmodifiable` (see [immutability](../02%20Advanced%20Dart/immutability.md)) |
...
6. **🔴 Since `_` is library-scoped, how do you get stricter boundaries?** — Put the class in its own library/file (and package `src/`) so no same-file code can reach its privates ([02 · libraries](../02%20Advanced%20Dart/libraries_and_packages.md)).
