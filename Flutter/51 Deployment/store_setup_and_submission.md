...
This file is the practical checklist for both consoles: account/app creation, metadata, assets (with sizing), the compliance/privacy forms, pricing/availability, and uploading a build to a track. It's the store-side execution of the fundamentals ([deployment_fundamentals.md](deployment_fundamentals.md)).
...
- **Compliance/privacy forms (mandatory — block submission if missing/wrong)** ([review_process_and_compliance.md](review_process_and_compliance.md)/[Module 37](../37%20Security/README.md)):
...
- **Build upload → track**:
  ...
  - Ensure the **build's version/build number** and **bundle id** match the console + are unique/increasing ([deployment_fundamentals.md](deployment_fundamentals.md)).
...
Not runtime perf — but **listing quality drives install conversion** (screenshots/description = ASO). App size (from the build) affects install rates ([app_size_and_build_optimization.md](app_size_and_build_optimization.md)). Automating submission speeds release cadence.
