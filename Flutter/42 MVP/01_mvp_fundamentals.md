...
- **Key differences from MVC**:
  ...
  - **Interface-mediated**: the Presenter talks to a **view interface**, enabling mocking ([03_mvp_testability.md](03_mvp_testability.md)).
...
Covered in [04_mvp_in_flutter_and_comparison.md](04_mvp_in_flutter_and_comparison.md) — imperative view-driving conflicts with Flutter's rebuild-from-state model.
...
Not a perf concern at this level; imperative updates are direct method calls. (Flutter-specific efficiency/friction is in [04_mvp_in_flutter_and_comparison.md](04_mvp_in_flutter_and_comparison.md).)
...
MVP's insight is the **explicit view contract**: routing all view updates through an interface makes presentation logic fully mockable and the view truly dumb. That testability idea is MVP's lasting contribution — but its **imperative presenter-drives-view** flow predates reactive UIs, which is why MVVM (achieving similar testability via observable state) fits Flutter better. Understand MVP for the contract/testability concept; reach for MVVM in practice ([02_presenter_and_view_contract.md](02_presenter_and_view_contract.md), [Module 43](../43%20MVVM/README.md)).
