...
- **Incremental build/CI (the headline win)**: with packages, a change recompiles/re-tests only the **changed package + its dependents**, not the whole app. Melos `--since` scopes analyze/test to affected packages ([monorepo_and_melos.md](monorepo_and_melos.md)) → **CI time scales with change size, not repo size**. Leaf features change often + cheaply; roots (core/contracts) rarely.
- **The stability corollary (critical)**: because **everything depends on `core`/`contracts`**, a change there **rebuilds/re-tests everything** — so keep them **stable, small, low-churn**. Volatile shared packages **destroy** the incremental benefit. Depend on stable things ([Module 04](../04%20SOLID/README.md) SDP).
...
- **When the payoff is real**: large repos (build pain), many teams (autonomy), shared/published packages (reuse). For small/single-team apps the versioning/ownership machinery is overhead ([modular_fundamentals.md](modular_fundamentals.md)).
