...
- **A module = a package**: each feature/core becomes a Dart/Flutter package (its own `pubspec.yaml`, `lib/`, tests). In a monorepo they're **path dependencies** ([monorepo_and_melos.md](monorepo_and_melos.md)).
- **Boundaries become structural (the key upgrade)**:
  ...
  - **Public API only**: consumers can only use what a package **`export`s** from its main library; everything else is effectively private ([module_boundaries_and_contracts.md](module_boundaries_and_contracts.md)).
...
- **Benefits over feature-first folders**:
  ...
  - **Incremental/parallel builds**: only changed packages (+ dependents) rebuild/re-test → faster CI at scale ([build_versioning_and_teams.md](build_versioning_and_teams.md)).
