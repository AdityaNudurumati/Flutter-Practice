...
- **Monorepo layout** ([02_monorepo_and_melos.md](02_monorepo_and_melos.md)): `apps/app` + `packages/{core, contracts, feature_auth, feature_cart}`; path deps; `melos.yaml` (globs + analyze/test/gen scripts).
- **Package graph (acyclic star)** ([03_module_boundaries_and_contracts.md](03_module_boundaries_and_contracts.md)): features → **core + contracts** (never each other); **app → everything** (to wire). Contracts/core are stable roots; features are leaves; app is the top.
...
- **Operational setup** ([04_build_versioning_and_teams.md](04_build_versioning_and_teams.md)): melos **bootstrap** + **`--since` incremental CI**; **CODEOWNERS** per package; **per-package versioning**; **stable core/contracts**.
