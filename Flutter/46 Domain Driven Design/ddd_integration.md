...
- **Strategic setup** ([ddd_fundamentals.md](ddd_fundamentals.md)): choose **one bounded context** (Ordering), write its **ubiquitous language** glossary; this context = a **feature/module** whose **domain layer** is this model.
- **Tactical model** ([tactical_building_blocks.md](tactical_building_blocks.md)): **value objects** (`Money`, `Quantity`, `Address` — immutable, self-validating), **entities** (`Customer`, `LineItem` — identity + behavior), a **domain service** (`PricingService`) for cross-object rules, **factories** for valid creation.
- **Aggregate + invariants** ([aggregates_and_invariants.md](aggregates_and_invariants.md)): the **`Order` aggregate** — root guards line items + status, enforcing invariants (total = sum of lines, only draft editable, valid transitions, non-empty place); small; references `Customer` by id.
- **Repository + events + app service** ([repositories_and_domain_events.md](repositories_and_domain_events.md)): **`OrderRepository`** interface (per aggregate root, whole-aggregate), **`OrderPlaced`** domain event, thin **`PlaceOrderService`** (load → `place()` → save → publish).
...
- **Proportionality (honest)**: this depth suits a **complex core domain**; a CRUD/generic subdomain should stay simple ([ddd_fundamentals.md](ddd_fundamentals.md)). DDD is applied **where it earns its cost**.
