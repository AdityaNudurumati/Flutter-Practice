...
This file covers the pivotal DDD pattern: aggregates as consistency boundaries, the aggregate root as the single entry point and invariant guardian, and the rules for sizing aggregates and referencing across them. It organizes the tactical building blocks ([tactical_building_blocks.md](tactical_building_blocks.md)) into consistency units.
...
- **Repositories are aggregate-granular**: you get/save **whole aggregates** through a repository per aggregate root ([repositories_and_domain_events.md](repositories_and_domain_events.md)).
...
- Enforce invariants **inside the boundary** (throw/`Result`); make repositories **aggregate-granular** ([repositories_and_domain_events.md](repositories_and_domain_events.md)); keep it **pure Dart** + unit-tested.
