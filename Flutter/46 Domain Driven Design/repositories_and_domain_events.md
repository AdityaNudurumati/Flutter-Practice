...
- **Domain event**: an **immutable fact** representing **something significant that happened** (`OrderPlaced(orderId, at)`, `FundsTransferred`). Named in **past tense** (ubiquitous language). An aggregate **records/emits** events as part of its behavior; a dispatcher **publishes** them after the aggregate is saved; **handlers react** (update another aggregate, notify, integrate). Events are the backbone of **eventual consistency** across aggregates/contexts (no giant cross-aggregate transaction — [aggregates_and_invariants.md](aggregates_and_invariants.md)).
