...
This file covers how to test a ViewModel (state-sequence assertions) and positions MVVM against MVC/MVP and alongside Clean Architecture, delivering the module's recommendation. It's the decision + verification context before the capstone ([mvvm_integration.md](mvvm_integration.md)).
...
- **Test by state sequence**: inject **fake use cases** (canned `Result`s), invoke **commands** (`search`, `retry`), and assert the **emitted state sequence** matches expectations (`[SearchLoading, SearchResults]`, `[SearchLoading, SearchError]`, `[SearchLoading, SearchEmpty]`). The sequence is the contract ([viewmodel_design.md](viewmodel_design.md)).
...
Not a runtime concern — this is about test speed (fast unit tier) and confidence. State-sequence tests are quick + stable, ideal for CI. The architecture's runtime perf depends on rebuild scoping ([data_binding_and_state.md](data_binding_and_state.md)).
