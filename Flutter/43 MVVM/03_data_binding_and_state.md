...
This file connects MVVM to concrete Flutter state tools, showing each as a binding mechanism, and focuses on the cross-cutting skill: scoping rebuilds for performance. It's the "how do I actually wire the binding" companion to the conceptual fundamentals ([01_mvvm_fundamentals.md](01_mvvm_fundamentals.md)).
...
- **They're the same MVVM shape**: each exposes **observable state** the View **watches** and **rebuilds** from, with **commands** (methods/events) — differing in ergonomics (boilerplate, compile-safety, event modeling), not architecture. Choose by team/needs ([Module 11](../11%20State%20Management/README.md)); the ViewModel design ([02_viewmodel_design.md](02_viewmodel_design.md)) stays the same.
...
- **Effects channel**: one-off effects (nav/snackbar) via `BlocListener`/`ref.listen`/a stream the View listens to — separate from the rebuild binding ([02_viewmodel_design.md](02_viewmodel_design.md)).
