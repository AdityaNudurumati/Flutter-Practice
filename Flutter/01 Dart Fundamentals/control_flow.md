...
This file covers conditionals (`if`/`else`), loops (`for`, `for-in`, `while`, `do-while`), `break`/`continue`, the modern `switch` (statement **and** expression), and collection-`if`/`for` used inside literals. Dart 3's pattern-enabled `switch` is a major upgrade covered here at an introductory level (full treatment in [records_and_patterns.md](records_and_patterns.md)).
