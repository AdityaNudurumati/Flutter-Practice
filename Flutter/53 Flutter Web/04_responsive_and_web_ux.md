...
- **Web conventions**: respect browser back/forward + URLs ([02_web_specific_concerns.md](02_web_specific_concerns.md)), tooltips on hover, standard cursors, selectable text, and don't reinvent native controls users expect.
...
The web engine reports pointer (mouse/hover/wheel/right-click) + keyboard events; the **semantics layer** produces an accessibility tree for the browser (screen readers) — critical since the canvas has no inherent DOM semantics ([01_web_fundamentals_and_renderers.md](01_web_fundamentals_and_renderers.md)).
...
Not primarily a perf topic; responsive rebuilds on resize should be efficient (scoped) and lists virtualized ([03_web_performance_and_loading.md](03_web_performance_and_loading.md)/[Module 21](../21%20Performance/README.md)). The concern is **UX quality + accessibility**, not speed. Semantics add negligible cost.
