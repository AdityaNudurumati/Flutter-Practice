...
Code hardening is the resilience layer (MASVS "Resilience"): obfuscation + integrity/attestation that raise attacker cost and feed **server-side** risk decisions. Architecturally it must be wired as *signals to the server*, not client gates, and paired with de-obfuscatable observability. Combined with the other layers (storage/network/server authz), it completes defense-in-depth — while the module's core truth holds: the server, not the hardened client, is the real defense ([security_model_and_owasp.md](security_model_and_owasp.md), [network_security_and_pinning.md](network_security_and_pinning.md), [security_integration.md](security_integration.md)).
