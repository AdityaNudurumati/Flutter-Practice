...
- **`csv`** (free): encode/decode delimited text (`ListToCsvConverter`/`CsvToListConverter`). No types/styling/multiple sheets — but universal, tiny, streamable ([csv_and_data_interchange.md](csv_and_data_interchange.md)).
...
- **XLSX = zipped XML (OOXML)**: a `.xlsx` is a ZIP of XML parts. Reading/writing decompresses/serializes lots of XML → **memory + CPU heavy** for large files → do it in an **isolate** ([02 · isolates](../02%20Advanced%20Dart/isolates_and_concurrency.md)); CSV is far lighter.
