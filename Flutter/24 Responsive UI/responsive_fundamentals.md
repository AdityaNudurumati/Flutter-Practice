...
- **Two mechanisms**:
  - **Fluid**: content stretches/shrinks continuously with available space (flex, fractions, constraints — [responsive_layout_widgets.md](responsive_layout_widgets.md)). No thresholds.
...
Not applicable; layout decisions are per-frame based on size ([07 · constraints_and_sizing](../07%20Widgets/constraints_and_sizing.md)).
...
Layout re-evaluation on resize/rotate is cheap; branching by tier is negligible. Avoid heavy work in size-dependent builders ([21 · rebuild_optimization](../21%20Performance/rebuild_optimization.md)).
