...
- **Text scaling / accessibility**: the OS/user font scale is in `MediaQuery.textScaler` (newer) / `textScaleFactor` (older). `Text` scales automatically; your **layout** must not assume fixed text heights — use flexible/wrapping layouts and avoid clipping. Don't disable scaling; **test at large scales**. Use theme text styles ([07 · text_and_theming](../07%20Widgets/06_text_and_theming.md)) and let them scale.
- **Responsive typography/spacing**: pick base sizes/spacing per size tier (larger on expanded), ideally from theme/tokens; scale gaps/paddings with a spacing helper. Cap line length for readability ([03_responsive_layout_widgets.md](03_responsive_layout_widgets.md)).
...
Not applicable; these are layout/metric-driven decisions per frame ([02_mediaquery_vs_layoutbuilder.md](02_mediaquery_vs_layoutbuilder.md)).
...
Insets/notches/keyboard come from the platform via the embedder into `MediaQuery` ([10 · embedder_and_startup](../10%20Flutter%20Architecture/05_embedder_and_startup.md)).
...
Negligible; these are layout choices. Rebuilds on text-scale/orientation/keyboard changes are expected — scope them ([21 · rebuild_optimization](../21%20Performance/02_rebuild_optimization.md)).
