...
Provider objects live in the tree (scope-bound); Riverpod state lives in the `ProviderContainer` (with `autoDispose` for cleanup) ([11 · riverpod](../11%20State%20Management/06_riverpod.md), [08 · dispose_and_leaks](../08%20Widget%20Lifecycle/05_dispose_and_leaks.md)).
...
DI access is cheap; Riverpod `autoDispose` bounds lifetime. No notable cost vs `get_it` ([11 · riverpod](../11%20State%20Management/06_riverpod.md)).
