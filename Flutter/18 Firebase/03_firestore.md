...
- **Reads**: `get()` (once) or **`snapshots()`** (`Stream` of live updates — [02 · streams](../02%20Advanced%20Dart/03_streams.md)); queries via `.where()/.orderBy()/.limit()` (composite queries need **indexes**; Firestore prompts to create them).
...
Snapshots hold document data in memory; offline cache persists locally. Large query results cost memory + read charges — paginate (`limit`, `startAfter`) ([15 · caching_strategies](../15%20Local%20Storage/05_caching_strategies.md)).

## Compiler Behavior

Documents are untyped maps by default; use converters (`withConverter`) or DTO mapping for type safety ([02 · json](../02%20Advanced%20Dart/12_json_and_serialization.md)).
