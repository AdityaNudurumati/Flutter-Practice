...
- **Methods**: `createUserWithEmailAndPassword`, `signInWithEmailAndPassword`, `signInWithCredential` (federated — combine with `google_sign_in`/`sign_in_with_apple` from [17 · oauth](../17%20Authentication/03_oauth_and_social_login.md)), `signInAnonymously`, phone auth, `sendPasswordResetEmail`, `signOut`.
- **Reactive state**: `authStateChanges()` (`Stream<User?>`) emits on sign-in/out/token refresh — feed it to your auth state / router `refreshListenable` ([17 · session_management](../17%20Authentication/05_session_management.md), [13 · guards](../13%20Routing/03_guards_and_redirects.md)).
...
`FirebaseAuth.instance` is a singleton; the current user + session are managed by the SDK (persisted natively). Wrap behind a repository ([05 · repository](../05%20Design%20Patterns/20_repository.md)).
...
- Drive **reactive auth state** from `authStateChanges()`; feed guards/UI ([13](../13%20Routing/03_guards_and_redirects.md), [17 · session_management](../17%20Authentication/05_session_management.md)).
...
- Combine federated sign-in with `google_sign_in`/`sign_in_with_apple` ([17 · oauth](../17%20Authentication/03_oauth_and_social_login.md)).
