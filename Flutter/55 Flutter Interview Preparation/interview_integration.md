...
- **1. Assess (current vs target)** ([interview_formats_and_prep.md](interview_formats_and_prep.md)): honestly rate yourself per round (DSA/conceptual/machine-coding/system-design/behavioral) against the **target level's bar** (junior/mid/senior). Gaps = where effort goes.
...
- **3. Per-round drills + banks + module links**:
  - **DSA**: pattern problems in Dart, timed, out loud ([dsa_in_dart.md](dsa_in_dart.md)).
  - **Conceptual**: rehearse the leveled Q&A bank (define→why→example→trade-offs), study linked modules ([flutter_dart_conceptual_questions.md](flutter_dart_conceptual_questions.md)).
  - **Machine coding**: timed feature builds (clean state/architecture) ([Module 56](../56%20Machine%20Coding%20Rounds/README.md)).
  - **System design**: the framework on common prompts ([Module 48](../48%20System%20Design/README.md)/[behavioral_and_system_design_rounds.md](behavioral_and_system_design_rounds.md)).
  - **Behavioral**: STAR story bank ([behavioral_and_system_design_rounds.md](behavioral_and_system_design_rounds.md)).
  - Each area **links back to the handbook modules** for depth.
- **4. Portfolio/resume** ([behavioral_and_system_design_rounds.md](behavioral_and_system_design_rounds.md)): 2-3 strong complete projects + impact-focused, backable resume bullets + clean GitHub — credibility backbone.
